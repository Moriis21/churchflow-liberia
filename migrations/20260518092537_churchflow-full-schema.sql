...
-- Multi-tenant: every table carries church_id for RLS isolation
-- ============================================================
...
CREATE POLICY "att_members_via_attendance" ON attendance_members
  FOR ALL USING (
    attendance_id IN (
      SELECT a.id FROM attendance a
      JOIN user_profiles up ON up.church_id = a.church_id
...
CREATE POLICY "finance_access_offerings" ON offerings
  FOR ALL USING (
    church_id IN (SELECT church_id FROM user_profiles WHERE id = auth.uid())
...
CREATE POLICY "finance_access_expenses" ON expenses
  FOR ALL USING (
    church_id IN (SELECT church_id FROM user_profiles WHERE id = auth.uid())
    AND (SELECT role FROM user_profiles WHERE id = auth.uid())
        IN ('super_admin','church_admin','pastor','treasurer','secretary')
...
CREATE POLICY "church_sermons_delete" ON sermons
  FOR DELETE USING (
    church_id IN (SELECT church_id FROM user_profiles WHERE id = auth.uid())
    AND (SELECT role FROM user_profiles WHERE id = auth.uid())
        IN ('super_admin','church_admin','pastor')
...
CREATE POLICY "church_sms_all" ON sms_logs
  FOR ALL USING (
    church_id IN (SELECT church_id FROM user_profiles WHERE id = auth.uid())
    AND (SELECT role FROM user_profiles WHERE id = auth.uid())
        IN ('super_admin','church_admin','pastor','secretary')
