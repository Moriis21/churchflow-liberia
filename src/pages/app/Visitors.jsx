...
import { VISITORS, MEMBERS } from '../../data/dummyData'
    ...
    <div className="bg-white rounded-xl border border-slate-100 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 p-4 cursor-pointer group">
      ...
      <div className="space-y-1.5 mb-3">
        <div className="flex items-center gap-1.5 text-xs text-slate-500">
          ...
          <span>Visited {formatDate(visitor.visitDate)}</span>
        </div>
        {visitor.address && (
          <div className="flex items-center gap-1.5 text-xs text-slate-500">
        ...
        <div className="flex items-center gap-1.5 text-xs text-slate-500">
          ...
          <span className="truncate">Assigned: {visitor.assignedTo}</span>
        </div>
      </div>
        ...
        {visitor.followUpStatus !== 'converted' && (
          <button
            onClick={() =>
              onStatusChange(
                visitor.id,
                visitor.followUpStatus === 'pending' ? 'contacted' : 'converted'
              )
            }
          ...
          >
            ...
            {visitor.followUpStatus === 'pending' ? 'Mark Contacted' : 'Convert'}
          </button>
        )}
      </div>
    </div>
...
// ─── Add Visitor Modal ────────────────────────────────────────
function AddVisitorModal({ isOpen, onClose, onSave }) {
...
function VisitorDetailModal({ visitor, isOpen, onClose, onStatusChange, onConvert }) {
  ...
  const [notes, setNotes] = useState(
    visitor
      ? [{ text: visitor.notes, time: formatDate(visitor.visitDate), system: true }]
      : []
  )

  // Reset notes when visitor changes
  const handleAddNote = () => {
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Visitor Details" size="xl">
      <div className="space-y-5">
        ...
        <div className="flex items-center gap-4 p-4 bg-gradient-to-r from-purple-50 to-violet-50 rounded-2xl border border-purple-100">
          ...
          <div className="flex-1 min-w-0">
            <h3 className="text-base font-bold text-[#1E1B4B]">{visitor.name}</h3>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-slate-50 rounded-xl p-3 border border-slate-100">
            ...
            <p className="text-sm font-bold text-slate-800 mt-1">{formatDate(visitor.visitDate)}</p>
          </div>
          <div className="bg-slate-50 rounded-xl p-3 border border-slate-100">
            <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide">Assigned To</p>
            <p className="text-sm font-bold text-slate-800 mt-1">{visitor.assignedTo || '—'}</p>
          </div>
...
export default function Visitors() {
  ...
  const handleStatusChange = (id, newStatus) => {
    setVisitors((prev) =>
      prev.map((v) => (v.id === id ? { ...v, followUpStatus: newStatus } : v))
    )
    if (selectedVisitor?.id === id) {
      setSelectedVisitor((prev) => ({ ...prev, followUpStatus: newStatus }))
    }
  }
  ...
  const filteredVisitors =
    filterStatus === 'all' ? visitors : visitors.filter((v) => v.followUpStatus === filterStatus)
        ...
        {viewMode === 'kanban' && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {(['pending', 'contacted', 'converted']).map((status) => {
              ...
              const colVisitors = visitors.filter((v) => v.followUpStatus === status)
              return (
                <div
                  key={status}
                  ...
                  {filteredVisitors.map((visitor) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-5 py-4 text-slate-600">{formatDate(visitor.visitDate)}</td>
                        <td className="px-5 py-4">
                          <StatusBadge status={visitor.followUpStatus} />
                        </td>
                        <td className="px-5 py-4 text-slate-600">{visitor.assignedTo || '—'}</td>
                        <td className="px-5 py-4">
                          <div className="flex items-center gap-2">
                            <button
