...
function ChurchProfileSection({ churchData }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Member Invite Link */}
      <div className="mt-6 p-4 rounded-xl border border-purple-100 bg-purple-50">
        <div className="flex items-center gap-2 mb-2">
          <Link2 className="w-4 h-4 text-purple-600" />
          <span className="text-sm font-semibold text-purple-900">Church Invite Link</span>
        </div>
        <p className="text-xs text-slate-500 mb-3">
          Share this link with your members so they can register and automatically join your church.
        </p>
        <div className="flex gap-2">
          <input
            readOnly
            value={`https://churchflowliberia.insforge.site/join/${churchData?.id || 'your-church-id'}`}
            className="flex-1 text-xs bg-white border border-purple-200 rounded-lg px-3 py-2 text-slate-700 focus:outline-none"
          />
          <button
            onClick={() => {
              navigator.clipboard.writeText(
                `https://churchflowliberia.insforge.site/join/${churchData?.id || 'your-church-id'}`
              )
...
export default function Settings() {
  ...
  const renderSection = () => {
    switch (activeSection) {
      case 'profile': return <ChurchProfileSection churchData={churchData} />
